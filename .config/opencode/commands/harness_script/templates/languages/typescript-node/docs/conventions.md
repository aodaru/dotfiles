...
- **Versión:** Node.js 18+ / TypeScript 5+.
- **Formato:** ESLint + Prettier + strict. Líneas máximo 100 caracteres.
- **Imports:** ESM (`import`/`export`). Una importación por línea.
...
| Genéricos               | `T`, `K`, `V`     | `TEntity` si hay varios |
...
import type { Something } from "./types.js";
...
- Usa `vitest` con `describe`/`it`/`expect`.
